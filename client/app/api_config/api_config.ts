export class API{

public static API_ENDPOINT =  "http://localhost:4000";

public static API_REGISTER = API.API_ENDPOINT + "/users";

public static API_AccessToken = API.API_ENDPOINT + "/oauth/token";

public static API_AddCustomer = API.API_ENDPOINT + '/api/addcustomer';

public static API_AddAreacode = API.API_ENDPOINT + '/api/addareacode';

public static API_UpdateAreacode = API.API_ENDPOINT + '/api/updateareacode/';

public static API_AddEmployee = API.API_ENDPOINT + '/api/addemployee';

public static API_UpdateEmployee = API.API_ENDPOINT + '/api/updateemployee/';

public static API_AddClient = API.API_ENDPOINT + '/api/addclient';

public static API_UpdateClient = API.API_ENDPOINT + '/api/updateclient/';

public static API_AddServicerequest = API.API_ENDPOINT + '/api/addservicerequest';

public static API_UpdateServicerequest = API.API_ENDPOINT + '/api/updateservicerequest/';

public static API_UpdateCustomer = API.API_ENDPOINT + '/api/updatecustomer/';

public static API_GetCustomer = API.API_ENDPOINT + '/api/getcustomer/';

public static API_GetEmployee = API.API_ENDPOINT + '/api/getemployee/';

public static API_GetAreacode = API.API_ENDPOINT + '/api/getareacode/';

public static API_GetEmployeePlan = API.API_ENDPOINT + '/api/getemployeeplan/';

public static API_GetServiceRequestPlan = API.API_ENDPOINT + '/api/getservicerequestplan/';

public static API_GetClients = API.API_ENDPOINT + '/api/getclients/';

public static API_GetServicerequest = API.API_ENDPOINT + '/api/getservicerequest/';

public static API_GetClientcodedetail = API.API_ENDPOINT + '/api/getclientcodedetails/';

public static API_RemoveCustomer = API.API_ENDPOINT + '/api/removecustomer/';

public static API_RemoveArea = API.API_ENDPOINT + '/api/removearea/';

public static API_RemoveService = API.API_ENDPOINT + '/api/removeservice/';

public static API_UpdatePassword = API.API_ENDPOINT + '/api/updatepassword';

}